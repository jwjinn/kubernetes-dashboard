...
const generateNpuDeviceHistory = () => {
    ...
    const buildSeries = (base: number, spread: number, transform?: (value: number) => number) =>
        STATIC_NPU_DEVICES.map((device, deviceIndex) => ({
            deviceId: device.id,
            node: device.node,
            uuid: device.uuid,
...
const STATIC_NPU_DEVICE_HISTORY = generateNpuDeviceHistory();
...
export const handlers = [
    ...
    http.get('/api/npu/device-history', () => {
        return HttpResponse.json(STATIC_NPU_DEVICE_HISTORY);
    }),
